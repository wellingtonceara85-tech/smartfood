...
/**
 * Garante ADR-0022: catalogo só consome outro Bounded Context através da Application Service
 * Interface exportada (application/use-cases/*) — nunca domain/infrastructure/api internos de
 * outro módulo. catalogo consome `BuscarLojaPorEmpresaUseCase` de identidade-empresa de
 * propósito (Missão 0011). Ver identidade-empresa/test/contract para o histórico da correção
 * (Missão 0013 — versões anteriores nunca detectavam violação real nenhuma).
 */
describe('Contrato — catalogo só consome outro Bounded Context via Use Case exportado', () => {
